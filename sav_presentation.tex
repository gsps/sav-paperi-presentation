...
\begin{frame}[fragile]{Motivation}
  ...
  \item<2->
    ...
    \todo{Replace with better example? No, keep it simple here for the overview, better example is too long and will be presented in second part}
  \end{itemize}
\end{frame}
...
\includegraphics[scale=0.6]{paper_figure_5.png}
...
\onslide<2->{Answer: $TVar \rightarrow Typ$}

Question: Why is $S$ not applied to $e$?

\onslide<3->{Answer: Because $e$ cannot contain type variables.}

Question: Don't we need another substitution lemma?

\onslide<4->{Answer: We also need a substitution lemma on the term level, to prove that if we step from $(\lambda x . e_1) v$ to $e_1[x \rightarrow v]$, the type is preserved.}
